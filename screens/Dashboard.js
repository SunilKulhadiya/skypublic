...
import {View, Text, StyleSheet, Dimensions, Image, ImageBackground, TouchableOpacity,
  StatusBar, Modal, ScrollView, FlatList, Linking} from "react-native";
import {FontAwesome, MaterialIcons, MaterialCommunityIcons, AntDesign,
        Foundation, Ionicons, FontAwesome5, Feather} from '@expo/vector-icons';
...
const Dashboard = ({ navigation, route }) => {
  ...
  async function AsyncData() {
    if(config.DEVICEHEIGHT <= 708){
      set_WhatsappVerPos(80);
    }
    if(config.DEVICEHEIGHT > 708 && config.DEVICEHEIGHT <= 734){
      set_WhatsappVerPos(70);
    }
    if(config.DEVICEHEIGHT > 734){
      set_WhatsappVerPos(60);
    }
    set_BirthWish(false);
    try {
  ...
  const OpenWhatsApp = async () => {
    try {
      const url = `whatsapp://send?phone=+919755766915&text=Hello Sky Public School, i have some query`;
  ...
  const MarqueeDisplay = (item, index) => {
    let str = item.description, wdth = 100, hgt = 100;
    str = str.toString();
    str = str.replace(/(<([^>]+)>)/ig, '');
          ...
          <View style={{flexDirection: "row", marginLeft: 15}}>
            <View style={{width: config.DEVICEWIDTH * 0.25, paddingRight: 12}}>
            {
              item.feature_image !== null && item.feature_image.length > 2 ? (
                  <Image source={{uri: item.feature_image}} style={{width: wdth, height: hgt, 
      ...
      <ImageBackground source={require("../assets/skyImg1.jpg")} style={styles.Imgcontainer}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View style={{ display: "flex"}}>
              ...
              <View style={{marginTop: "0.5%", marginBottom: "1%"}}>
                <MarqueeView>
              ...
              <View style={styles.CardView1}>
                <View style={styles.MColumn}>
                  ...
                  <View style={styles.SRow}>
                    ...
                    <View style={styles.SColumn}>
                      <TouchableOpacity onPress={() => navigation.navigate('ViewDownload')}>
                        <View style={styles.IconsBackGround}>
                ...
                <View style={[styles.SRow, { marginLeft: 2, }]}>
                  ...
                  <View style={styles.SColumn}>
                    <TouchableOpacity onPress={() => navigation.navigate('SocialMedia')}>
                      <View style={styles.IconsBackGround}>
                      <Image source={require('../assets/socialmedia.png')} style={{
                        width: 60, height: 50, borderRadius: 12,}} />
                      </View>
                      <View style={[styles.SSColumn, { marginTop: -5 }]}>
                        <Text style={styles.textStyle}>Social</Text>
                        <Text style={styles.textStyle}>Media</Text>
                      </View>
                    </TouchableOpacity>
                  </View>
                  <View style={styles.SColumn}>
                    <TouchableOpacity onPress={() => navigation.navigate('Complain')}>
                      <View style={styles.IconsBackGround}>
                        <Feather name="alert-octagon" size={40} color="#240569" />
                      </View>
                      <View style={[styles.SSColumn, { marginTop: -5 }]}>
                        <Text style={styles.textStyle}>Complain</Text>
                      </View>
                    </TouchableOpacity>
                  </View>
                </View>
                    ...
                    <View style={styles.CardView2}>
                      <View style={styles.MColumn}>
                        ...
                        <View style={styles.SRow}>
                          ...
                          <View style={styles.SColumn}>
                            <View style={styles.IconsBackGround}>
                              <MaterialCommunityIcons name="bus-school" size={50} color="#E6DE0B" />
                            </View>
                            <View style={styles.SSColumn}>
                          ...
                          <View style={styles.SColumn}>
                            <View style={styles.IconsBackGround}>
                              <MaterialCommunityIcons name="notebook" size={50} color="#CDB2B2" />
                            </View>
                            <View style={styles.SSColumn}>
...

{
  ShowBirthGif ? (
    <Modal animationType="fade" transparent={true} visible={BirthWish} 
      ...
      <View style={styles.viewWrapper}>
        <View style={styles.BirthCardViewModel}>
          <View style={{flexDirection: "row", width: config.DEVICEWIDTH, height: config.DEVICEHEIGHT * 0.2}}>
    ...
    <></>
          ...
          <TouchableOpacity
              style={[styles.touchableOpacityStyle, {width: 55, height: 55,
              bottom: config.DEVICEHEIGHT <=734 ? "11%": "7%", left: "1%"}]}
              activeOpacity={0.7}
              onPress={() => OpenWhatsApp()}>
            <Ionicons name="chatbubble-ellipses" size={45} color="grey"
                style={styles.floatingB} />
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.touchableOpacityStyle, {width: 55, height: 55,
            bottom: config.DEVICEHEIGHT <=734 ? "11%": "8%", left: "86%"}]}
            activeOpacity={0.7}
            onPress={() => OpenWhatsApp()}>
              <Image source={require('../assets/WhatsApp_Icon_balloonWhite.png')}
                style={[styles.floatingB,{width: 55, height: 55,}]} />
          </TouchableOpacity>
      </ImageBackground>
...
const styles = StyleSheet.create({
  ...
  TopBackground1: {
    width: config.DEVICEWIDTH,
    height: config.DEVICEHEIGHT * 0.275,
  },
  MessageAlign: {
  ...
  birthdayGif: {
    width: config.DEVICEWIDTH * 0.943,
  ...
  CardView2: {
    ...
    backgroundColor: "#FFFFFF",
    borderRadius: 7,
    marginBottom: config.DEVICEHEIGHT * 0.0,
  },
  MColumn: {
    flexDirection: 'column',
